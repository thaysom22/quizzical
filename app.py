...
def register():
    ...
    # POST #
    # form input validation done client-side
    username_form = request.form.get("username").lower()
    existing_user = mongo.db.users.find_one(
        {
            "username": username_form
        }
...
def discover():
    ...
    # credit for "$arrayElemAt" workaround:
    # https://stackoverflow.com/questions/37691727/how-to-use-mongodbs-aggregate-lookup-as-findone
    quizzes_by_category = {}
    for category in all_categories:
        quizzes_by_category[category["_id"]] = list(
            mongo.db.quizzes.aggregate([
                {
                    "$match": {
                        "quiz_category_id": category["_id"]
                    }
                },
                {
                    "$sample": {
                ...
                },
                {
                    "$addFields": {
                        ...
                        # operation causes the original field to be replaced
                        "quiz_age_range_data": {
                            "$arrayElemAt": ["$quiz_age_range_data", 0]
                        },
                        "quiz_owner_data": {
                            "$arrayElemAt": ["$quiz_owner_data", 0]
                        }
                    }
                },
                {
    ...
    for age_range in all_age_ranges:
        quizzes_by_age_range[age_range["_id"]] = list(
            mongo.db.quizzes.aggregate([
    ...
    recc_quizzes_by_category = list(mongo.db.quizzes.aggregate([
        {
            "$match": {
                "quiz_category_id": ObjectIdHelper.toObjectId(user_category_id)
            }
        },
        {"$sample": {"size": 3}},
        {
    ...
    recc_quizzes_by_age_range = list(mongo.db.quizzes.aggregate([
        {
            "$match": {
                "quiz_age_range_id": ObjectIdHelper.toObjectId(user_age_range_id)
            }
        },
        {"$sample": {"size": 3}},
        {
    ...
    # dict comp. to remove duplicates, CREDIT:
...
def search():
    ...
    if request.method == "GET":
        ...
        if all_quizzes == "true":
            ...
            # CREDIT:
            # https://stackoverflow.com/questions/59918113/mongodb-aggregate-match-a-document-or-all
            ...
            # remove duplicates
            search_results = list(
                {item["_id"]: item for item in search_results}.values())
    ...
    if request.method == "POST":
        ...
        # CREDIT for constructing search with text index:
    ...
    insert_quiz_result = mongo.db.quizzes.insert_one(
        {
            ...
            "quiz_owner_id": ObjectIdHelper.toObjectId(
                session.get('user').get('_id')),
            "quiz_category_id": ObjectIdHelper.toObjectId(
                request.form.get('create_quiz_category')),
            "quiz_age_range_id": ObjectIdHelper.toObjectId(
                request.form.get('create_quiz_age_range')),
            "questions": []})
...
def view_quiz(view_quiz_id):
    ...
    # view-quiz.html template needs to know if current user is owner of viewed
    # quiz
    user_is_owner = (
...
def delete_quiz(delete_quiz_id):
    ...
    # delete all associated from questions collection using
    # quiz_data.questions ids
    ...
    if num_questions > 0:
        ...
        if (not delete_questions_result) or (
                not delete_questions_result.deleted_count == num_questions):
            flash(
...
def add_question(quiz_id):
    ...
    if request.method == "POST":
        ...
        insert_result = mongo.db.questions.insert_one(
            {
                ...
                # convert to int index for answers list
        ...
        if not insert_result:
            ...
            # update questions field of quiz document in quizzes collection
            # identified by new_quiz_id
            update_quiz_result = mongo.db.quizzes.update_one(
                {"_id": ObjectIdHelper.toObjectId(quiz_id)},
            ...
            if update_quiz_result:
                flash(
                    f"Question {num_questions+1} was added to {quiz_title}.",
                    "success")
            else:
...
def delete_question(quiz_id, delete_question_id):
    ...
    if not user_is_owner:
        flash(
            "Question not deleted - user is not the quiz owner.",
            "invalid-action")
        return redirect(url_for(
            'view_quiz',
            view_quiz_id=quiz_id,
    ...
    # if quiz_id questions array is updated successfully, try to delete
    # question
